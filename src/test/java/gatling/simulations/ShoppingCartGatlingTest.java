 ...
 */
public class ShoppingCartGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all shoppingCarts").get("/api/shopping-carts").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new shoppingCart")
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created shoppingCart").get("${new_shoppingCart_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created shoppingCart").delete("${new_shoppingCart_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the ShoppingCart entity").exec(scn);
