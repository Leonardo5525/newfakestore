...
describe('CustomerDetails e2e test', () => {
  ...
  // const customerDetailsSample = {"gender":"FEMALE","phone":"+351 916293245","addressLine1":"snarling","city":"Tavira","country":"Guiana"};
  ...
  beforeEach(() => {
    ...
    cy.authenticatedRequest({
      method: 'POST',
      url: '/api/users',
      body: {"login":"battle visor positive","firstName":"Alexandre","lastName":"Henriques"},
    }).then(({ body }) => {
      user = body;
    });
  });
  ...
  describe('new CustomerDetails page', () => {
    ...
    it.skip('should create an instance of CustomerDetails', () => {
      ...
      cy.get(`[data-cy="phone"]`).type('+351 937823840');
      cy.get(`[data-cy="phone"]`).should('have.value', '+351 937823840');

      cy.get(`[data-cy="addressLine1"]`).type('safely stage mussel');
      cy.get(`[data-cy="addressLine1"]`).should('have.value', 'safely stage mussel');

      cy.get(`[data-cy="addressLine2"]`).type('muted in ketch');
      cy.get(`[data-cy="addressLine2"]`).should('have.value', 'muted in ketch');

      cy.get(`[data-cy="city"]`).type('Vila Nova de Gaia');
      cy.get(`[data-cy="city"]`).should('have.value', 'Vila Nova de Gaia');

      cy.get(`[data-cy="country"]`).type('México');
      cy.get(`[data-cy="country"]`).should('have.value', 'México');
