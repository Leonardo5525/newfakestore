...
describe('ProductOrder e2e test', () => {
  ...
  // const productOrderSample = {"quantity":2079,"totalPrice":4159.26};
  ...
  beforeEach(() => {
    ...
    cy.authenticatedRequest({
      method: 'POST',
      url: '/api/products',
      body: {"name":"balance","description":"within closet","price":6271.96,"productSize":"XXL","image":"Li4vZmFrZS1kYXRhL2Jsb2IvaGlwc3Rlci5wbmc=","imageContentType":"unknown"},
    }).then(({ body }) => {
      product = body;
    });
    // create an instance at the required relationship entity:
    cy.authenticatedRequest({
      ...
      body: {"placedDate":"2023-12-13T04:39:06.832Z","status":"PENDING","totalPrice":29397.47,"paymentMethod":"CREDIT_CARD","paymentReference":"badly merrily"},
    }).then(({ body }) => {
      shoppingCart = body;
    });
  });
  ...
  describe('new ProductOrder page', () => {
    ...
    it.skip('should create an instance of ProductOrder', () => {
      cy.get(`[data-cy="quantity"]`).type('13805');
      cy.get(`[data-cy="quantity"]`).should('have.value', '13805');

      cy.get(`[data-cy="totalPrice"]`).type('20472.08');
      cy.get(`[data-cy="totalPrice"]`).should('have.value', '20472.08');
